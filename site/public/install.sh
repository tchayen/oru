...
resolve_version() {
  ...
  if command -v curl >/dev/null 2>&1; then
    VERSION="$(curl -fsSL https://registry.npmjs.org/oru/latest | grep -o '"version":"[^"]*"' | head -1 | cut -d'"' -f4)"
  elif command -v wget >/dev/null 2>&1; then
    VERSION="$(wget -qO- https://registry.npmjs.org/oru/latest | grep -o '"version":"[^"]*"' | head -1 | cut -d'"' -f4)"
  else
